...
from (Select Name, row_number() over (partition by occupation order by name) AS id 
      from Occupations 
      where Occupation = 'Doctor') D
full outer join (Select Name, row_number() over (partition by occupation order by name) AS id 
                 from Occupations 
